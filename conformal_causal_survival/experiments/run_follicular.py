...
from conformal_causal_survival.data.simulate_treatment import generate_treatment_follicular
...
def run_follicular_experiment():
    print("="*40, flush=True)
    print("RUNNING FOLLICULAR EXPERIMENT (n=541)", flush=True)
    print("="*40, flush=True)
    
    print("Loading Follicular Dataset...", flush=True)
    loader = DatasetLoader()
    try:
        data = loader.load_follicular()
    except Exception as e:
        print(f"Failed to load dataset: {e}", flush=True)
        return

    # Treatment logic:
    # If using CSV fallback, 'treatment' column might be created in load_follicular.
    # Check if we need to simulate or use existing.
    if 'treatment' not in data.columns:
        print("Generating Treatment (Simulation)...", flush=True)
        data['treatment'], _ = generate_treatment_follicular(data)
    
    print("Creating Splits...", flush=True)
    train_data, calib_data, test_data = create_splits(data)

    # Key features
    feature_cols = ['age_years', 'hemoglobin', 'stage', 'treatment']
    ...
    try:
        ...
        model_ccomp = CauseSpecificHazardModel(cause=COMPETING_CAUSE, features=feature_cols, model_type='cox')
        model_ccomp.fit(train_data)
    ...
    print("-" * 30, flush=True)
    print("RESULTS: FOLLICULAR", flush=True)
    print("-" * 30, flush=True)
